...
using Soenneker.Close.Runners.OpenApiClient.Utils.Abstract;
...
namespace Soenneker.Close.Runners.OpenApiClient.Utils;
...
public sealed class FileOperationsUtil : IFileOperationsUtil
{
    ...
    public async ValueTask Process(CancellationToken cancellationToken = default)
    {
        ...
        string openApiDocumentUrl = _configuration["Close:ClientGenerationUrl"] ?? "https://api.close.com/api/openapi.json";
        ...
        if (filePath == null)
            throw new InvalidOperationException("Close OpenAPI document download failed.");
        ...
        await _kiotaUtil.Generate(fixedFilePath, "CloseOpenApiClient", Constants.Library, gitDirectory, cancellationToken).NoSync();
    ...
    private async ValueTask BuildAndPush(string gitDirectory, CancellationToken cancellationToken)
    {
        ...
        if (!successful)
        {
            throw new InvalidOperationException("The generated Close client failed its Release build.");
        }
